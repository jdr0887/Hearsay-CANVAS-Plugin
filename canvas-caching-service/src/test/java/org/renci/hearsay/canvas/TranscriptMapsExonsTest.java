...
public class TranscriptMapsExonsTest {
    ...
    public void testPlusStrand() {
        ...
        try {
            ...
            List<TranscriptRefSeq> results = persistTranscriptRunnable.call();

            for (TranscriptRefSeq transcript : results) {

                assertTrue(transcript.getAccession().equals("XM_005277470.1"));
                Set<TranscriptAlignment> transcriptAlignmentSet = transcript.getAlignments();
                for (TranscriptAlignment transcriptAlignment : transcriptAlignmentSet) {
                    ...
                    sortedRegionSet.addAll(transcriptAlignment.getRegions());
                    for (Region region : sortedRegionSet) {
                        System.out.println(region.toString());
                    }
                    System.out.println(transcriptAlignment.toString());
                    if (transcriptAlignment.getStrandType().equals(StrandType.PLUS)) {
        ...
        try {
            ...
            for (TranscriptRefSeq transcript : results) {
                ...
                for (TranscriptAlignment transcriptAlignment : transcriptAlignmentSet) {
                    ...
                    System.out.println(transcriptAlignment.toString());
                    if (transcriptAlignment.getStrandType().equals(StrandType.MINUS)) {
