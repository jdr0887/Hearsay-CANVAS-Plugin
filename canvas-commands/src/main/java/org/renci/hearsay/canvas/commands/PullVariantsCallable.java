...
public class PullVariantsCallable implements Callable<Void> {
    ...
    public PullVariantsCallable(CANVASDAOBeanService canvasDAOBeanService, HearsayDAOBeanService hearsayDAOBeanService) {
        super();
    ...
    public Void call() throws HearsayDAOException {
        ...
        if (StringUtils.isNotEmpty(geneName)) {
            genes.addAll(hearsayDAOBeanService.getGeneDAO().findBySymbol(geneName));
        } else {
            genes.addAll(hearsayDAOBeanService.getGeneDAO().findAll());
        }
